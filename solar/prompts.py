...
# CRITICAL: FOL Syntax and Predicate Mapping

The FOL parser (NLTK) has strict constraints. Corpus tags like STEM-PST-3SG
are decomposed into atomic ABox assertions by the parser. Your FOL rules
MUST use those same atomic predicates — never raw tag strings.

## Tag → Predicate mapping (use THESE in fol_expression):

| Corpus tag part    | FOL predicate + value                              |
|--------------------|----------------------------------------------------|
| PST                | hasCandidateTense(A, "Past")                       |
| PRS / PRS.3SG      | hasCandidateTense(A, "Present")                    |
| PRS.12             | hasCandidateTense(A, "Present") & hasCandidatePerson(A, "12") |
| FUT                | hasCandidateTense(A, "Future")                     |
| 3SG                | hasCandidatePerson(A, "3") & hasCandidateNumber(A, "Singular") |
| 3PL                | hasCandidatePerson(A, "3") & hasCandidateNumber(A, "Plural")  |
| 1SG                | hasCandidatePerson(A, "1") & hasCandidateNumber(A, "Singular") |
| PASS               | hasCandidateVoice(A, "Passive")                    |
| CAUS               | hasCandidateVoice(A, "Causative")                  |
| IMP                | hasCandidateMood(A, "Imperative")                  |
| DEB                | hasCandidateMood(A, "Debitive")                    |
| ITER               | hasCandidateAspect(A, "Iterative")                 |
| INF                | hasCandidateVerbForm(A, "Infinitive")              |
| CVB                | hasCandidateVerbForm(A, "Converb")                 |
| PTCP.ACT           | hasCandidateVerbForm(A, "Participle") & hasCandidateVoice(A, "Active") |
| PTCP.PST           | hasCandidateVerbForm(A, "Participle") & hasCandidateVoice(A, "Passive") |
| VN                 | hasCandidateVerbForm(A, "VerbalNoun")              |
| EVID               | hasCandidateVerbForm(A, "Evidential")              |
| NEG.SG / NEG.PL    | hasCandidatePolarity(A, "Negative")                |
| GEN/DAT/ACC/LOC... | hasCandidateCase(A, "Genitive") etc.               |
| ATTR               | hasCandidateVerbForm(A, "Attributive")             |

## Hard syntax rules:

1. Variable names: single uppercase letters ONLY: A, B, C, T, S, V, H, N, X
   WRONG: A_N, A_T, V_prev
   RIGHT: B, C, X

2. No dots or hyphens inside string literals.
   WRONG: hasCandidateTense(A, "PRS.12"), hasCandidateVoice(A, "PASS-PST")
   RIGHT: hasCandidateTense(A, "Present") & hasCandidatePerson(A, "12")

3. No ~ negation. Use positive properties only.
   WRONG: ~hasNegativeAuxiliary(C)
   RIGHT: Add a property hasNegativeAuxiliary with positive counterpart,
          or omit the negation condition entirely.

4. exists must use a single letter variable:
   RIGHT: exists B (candidateOf(B, N) & hasCandidatePOS(B, "Noun"))
